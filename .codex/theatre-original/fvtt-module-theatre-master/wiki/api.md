The api is reachable from the variable `game.modules.get('theatre').api` or from the socket library `socketLib` on the variable `game.modules.get('theatre').socket` if present and active.
